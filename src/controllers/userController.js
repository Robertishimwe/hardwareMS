...
class UsersController {
  ...
  static getAllUsers = async (req, res) => {
    try {
      ...
      const filteredUsers = users.filter(
        (user) => user.email !== "robert@ishimwe.rw"
      );
  ...
  static updateUserProfile = async (req, res) => {
    try {
      ...
      if (
        updateData.hasOwnProperty("password") &&
        role !== "admin" &&
        id !== userId
      ) {
        return res
          .status(403)
          .json({ message: "You are not authorized to modify the password" });
      }
      ...
      if (updateData.hasOwnProperty("role") && role !== "admin") {
        return res
          .status(403)
          .json({ message: "You are not authorized to modify the role" });
      }
